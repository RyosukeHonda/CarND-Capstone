...
class TLDetector(object):
    ...
    def project_to_image_plane(self, point_in_world):
        ...
        x, y, z = np.dot(np.dot(camera_intrinsic, rot_trans),
                         np.array([point_in_world.x, point_in_world.y, point_in_world.z, 1.0]))
    ...
    def nearest_waypoint_from_light(self, light_position, waypoints):
        ...
        for index, waypoint in enumerate(waypoints):
            dist = math.sqrt((light_position[0] - waypoint.pose.pose.position.x) ** 2 + (
            light_position[1] - waypoint.pose.pose.position.y) ** 2)
            if dist < min_dist:
                min_dist = dist
                min_index = index
        return min_index
